...
begin
  ...
  if (RadioGroup1.ItemIndex = 0) then begin
   ...
   Except
    ShowMessage('..: ATENÇÃO :.. ' + #13 + #10 + 'Período informado não está correto');
    Result := False;
   End;
  ...
  // coo e crz
   ...
   Except
    ShowMessage('..: ATENÇÃO :.. ' + #13 + #10 + 'Número informado não está correto');
    Result := False;
   End;
...
begin
 ...
 if (RadioGroup2.ItemIndex = 0) then begin
  ShowMessage('Opção válida apenas para: Tipo de Emissão = ARQUIVO');
  exit;
 end;
 ...
 if qryTabpro.IsEmpty then begin
  Application.MessageBox( 'Não existe registro!','GBPAF');
  exit;
 end;
 ...
 while not qryTabpro.Eof do begin
  ...
  // md5 produto
  // OBS: deve ser concatenado na mesma ordem da gravação do registro.
  slTabProMD5 :=
  ...
  P2.IPPT           := qryTabPro.FieldByName('ProdPropria').AsString;           // Produção Própria ou de Terceiro relativo à mercadoria
  ...
  if qryTabPro.FieldByName('TipTrb').AsString = 'TB' Then begin          // Tributado
   ...
   P2.ST    := 'II';
   P2.ALIQ  := 0;
  end else begin    // Subst.Trib ; Isento ; Não Incidência
 ...
 Except
   Application.MessageBox( pchar('O Arquivo não foi assinado!' + #13 + #10 +
...
begin
 ...
 // coo e crz
  ...
  ShowMessage('Opção válida apenas para: Filtrar por = DATA');
  exit;
 end;
 ...
 Try
  ...
  if qryMovRDZ.IsEmpty then begin
   Application.MessageBox( 'Não existe registro para o período selecionado!','GBPAF');
   exit;
  end;
 ...
 while not qryMovRDZ.Eof do begin
  ...
  with FrTelIni.ACBrPAF.PAF_R.RegistroR02.New do begin
   ...
   PAR_ECF     := 'N';                 // Parâmetro do ECF para incidência de desconto sobre itens sujeitos
   ...
   // 02 - Substituição Tributária-ICMS Fn / Isento-ICMS In / Não-incidência-ICMS Nn
   //      Substituição Tributária-ISSQN FSn / Isento-ISSQN Isn / Não-incidência-ISSQN NSn

   //-- ATENÇÃO: O índice será fixado como padrão o valor = 1
  ...
  with FrTelIni.ACBrPAF.PAF_R.RegistroR04.New do begin
   ...
   while not qryIteCNF.Eof do begin
    ...
    with RegistroR05.New do begin
     ...
     QTDE_ITEM    := qryIteCNF.FieldByName('QtdePro').AsFloat; //  sem a separação das casas decimais.

     UN_MED       := qryIteCNF.FieldByName('IdTabUnd').AsString;

     VL_UNIT      := qryIteCNF.FieldByName('ValPro').AsFloat;  //  sem a separação das casas decimais.
     ...
     if qryIteCNF.FieldByName('TipTrb').AsString = 'TB' Then begin
      ...
      COD_TOT_PARC := 'I1';
     end else if qryIteCNF.FieldByName('TipTrb').AsString = 'FF' Then begin // Substituição Tributária
      COD_TOT_PARC := 'F1';
     end else begin   // NN - Não Incidência
      COD_TOT_PARC := 'N1';
     end;
     ...
     QTDE_CANC    := qryIteCNF.FieldByName('Qtdecancelada').AsFloat;            //  sem a separação das casas decimais.
     VL_CANC      := qryIteCNF.FieldByName('Valcancelado').AsFloat;             // Valor cancelado, no caso de cancelamento parcial do item

     VL_CANC_ACRES:= 0;                                                         // valor do cancelamento de acréscimo no item
     IAT          := qryIteCNF.FieldByName('ArredTrunca').AsString;             // Arredondamento ou Truncamento

     if qryIteCNF.FieldByName('ProdPropria').AsString = 'S' then                // Produção Própria ou de Terceiro relativo à mercadoria
      IPPT        := 'P'
     else
      IPPT        := 'T';
 ...
 while not qryMovCNF.Eof do begin
  with FrTelIni.ACBrPAF.PAF_R.RegistroR06.New do begin
   NUM_USU     := 1;                                                            // Número de ordem do usuário do ECF              // VERIFICAR
   COO         := qryMovCNF.FieldByName('COO').AsInteger;                       // COO
   GNF         := 0;                                                            // Contador Geral de Operação Não Fiscal - QDO HOUVER
   GRG         := 0;                                                            // Informar apenas no caso de Relatório Gerencial
   CDC         := 0;                                                            // Informar apenas no caso de Comprovante Crédito ou Débito
   DENOM       := 'RV';                                                         // Registro de Venda
   DT_FIN      := qryMovCNF.FieldByName('DataInicialEmissao').AsDateTime;       // Data final de emissão (impressa no rodapé do documento)
   HR_FIN      := qryMovCNF.FieldByName('HoraInicialEmissao').AsDateTime;       // Hora final de emissão (impressa no rodapé do documento)
   ...
   while not qryPagCNF.Eof do begin
    ...
    with RegistroR07.New do begin
     ...
     IND_EST     := 'N';   // O SISTEMAS FOI PROGRAMADO PARA NÃO FAZER ESTORNO
     VL_EST      := 0;     // O SISTEMAS FOI PROGRAMADO PARA NÃO FAZER ESTORNO
...
begin
 ...
 ShowMessage('Este PAF-ECF não executa funções de baixa de estoque com base em índices técnicos de produção,' + #13 + #10 +
             'não podendo ser utilizado por estabelecimento que necessite deste recurso.');
...
begin
 ...
 if (RadioGroup2.ItemIndex = 1) then begin
  ShowMessage('Opção válida apenas para: Tipo de Emissão = IMPRESSO');
  exit;
 end;
...
begin
 ...
 // REQ. XXX - Relatório Gerencia denominado "MEIOS DE PAGAMENTO"

 // falta identificar/parametrizar no sistema o indice abaixo
 // pde ser que em alguns casos o índice "Gerencial X" seja diferente de 1
 ...
 Linha     := AnsiString(('Periodo: ' + DateToStr(DateTimePicker1.Date) + ' até ' + DateToStr(DateTimePicker2.Date)));
 FrTelIni.CompECF.LinhaRelatorioGerencial( AnsiString(Linha), 1 );
...
begin
 ...
 if (RadioGroup2.ItemIndex = 0) then begin
  Application.MessageBox('Operação realizada com sucesso','GBPAF');
 end else begin
...
begin
 ...
 if (RadioGroup2.ItemIndex = 0) then begin
  ShowMessage('Opção válida apenas para: Tipo de Emissão = ARQUIVO.'   + #13 + #10 +
              'Para gerar arq. eletrônico da Memória de Fita Detalhe,' + #13 + #10 +
...
begin
 ...
 // REQ. XLIII - Relatório Gerencia denominado "IDENTIFICAÇÃO DO PAF-ECF"
 ...
 if FileExists(FrTelIni.spDir + '\Cfg\ArqAuxEcf.cfg') then begin
   ...
   // Carregar informações do arquivo auxiliar
   ...
   While not Eof(F) do Begin
    ...
    // Se a 21ª posicao da linha for "|" então nessa linha existe informações do numero da ECF
...
begin
 ...
 if dlDatFin < DlDatIni then
  begin
   ...
   Application.MessageBox( 'Data Final não pode ser Menor que Data Inicial.', 'Sisa', MB_ICONERROR );
   Exit;
  end;

 // 3. Meses devem ser iguais
 if nData( dlDatIni, 'M' ) <> nData( dlDatFin, 'M' ) Then
  MsgErro( 'Os Meses do Período devem ser Iguais  !', DateTimePicker2 );

 // 4. Anos devem ser iguais
 if nData( dlDatIni, 'Y' ) <> nData( dlDatFin, 'Y' ) Then
  MsgErro( 'Os Anos do Período devem ser Iguais  !', DateTimePicker2 );

 {
 REQUISITO XXVIII
 ITEM 3: O arquivo gerado deverá ser denominado pelo nº laudo emitido pelo órgão técnico que
...
// qryMovNot.SQL.SaveToFile('c:\sql.txt');
 ...
 While not qryMovNot.Eof Do
  ...
  // Gerando Registros Tipo 74 (Inventário)
...
begin
 ...
 // 1. BLOCO 0 - Abertura, Identificação e Referencias

 // 1.1. Registro 0000 - Abertura do Arquivo e Identificação da Entidade

 iLin := 1;
 iLnG := 1;

 sAux := '|'  + '0000'  + '|';                                                            // Tipo de Registro
 sAux := sAux + StrZero(qryTabEmpCodVerEFD.AsString,3)                     + '|';         // Versão do Arquivo {Parametrizar};
 sAux := sAux + IntToStr( RadioGroup5.ItemIndex )                          + '|';         // Finalidade do Arquivo
 sAux := sAux + FormatDateTime( 'ddmmyyyy', dlDatIni )                     + '|';         // Inicio das Informações
 sAux := sAux + FormatDateTime( 'ddmmyyyy', dlDatFin )                     + '|';         // Final  das Informações
 sAux := sAux + Trim( qryTabEmpNomEmp.AsString )                           + '|';         // Razão Social da Entidade {100}
 ...
 sAux := sAux + Trim( OnlyNumbers( qryTabEmpInsEst.AsString ) )            + '|';         // Inscrição Estadual {14}
 sAux := sAux + StrZero( qryTabEmpCodIBGE.AsString, 07 )                   + '|';         // Codigo Municipio Conf. Tabela IBGE {CostaRica='5003256'}
 sAux := sAux + Trim( qryTabEmpInsMun.AsString )                           + '|';       // OC  // Inscrição Municipal 'Tamanho Indefinido' {15}
 sAux := sAux + ''                                                         + '|';       // OC  // Inscrição Suframa
 ...
 sAux := sAux +    Trim( qryTabEmpEndereco.AsString )               + '|';                // Endereço {60}
 ...
 if qryTabEmpInsEstSbt.AsString <> '' Then
  Begin
   ...
   sAux := sAux +        Trim( qryTabEmpInsEstSbt.AsString )     + '|';                   // Inscrição ST {14}
 ...
 While Not qryTabPrt.Eof Do
  Begin
   ...
   sAux := sAux +        Trim( qryTabPrtInsEst.AsString )                + '|';                             // OC // Inscrição Estadual {14}
   sAux := sAux + Se( qryTabPrtCodIBGE.AsInteger > 0, StrZero( qryTabPrtCodIBGE.AsString, 07 ), '' ) + '|'; // OC // Codigo Municipio Conf. Tabela IBGE
   sAux := sAux +        Trim( qryTabPrtSuframa.AsString )               + '|';                             // OC // Inscrição Suframa {09}
   sAux := sAux +        Trim( qryTabPrtEndPrt.AsString )                + '|';                             // Endereço {60}
 ...
 // 1.7. Registro 0190 - Identificação das Unidades de Medida
 ...
 While Not qryTabEmb.Eof Do
  Begin
   ...
   sAux := sAux + Trim( qryTabEmbNomEmb.AsString )                       + '|';                             // Descrição da Embalagem {Tamanho Indefinido}
 ...
 // 1.8. Registro 0200 - Tabela de Identificação do Item (Produtos e Serviços)
 ...
 While Not qryTabProSped.Eof Do
  Begin
   ...
   sAux := sAux +    Trim( qryTabProSpedNomPro.AsString )                    + '|';                         // Descrição do Produto 'NomPro + Modelo' {Tamanho Indefinido}
   ...
   sAux := sAux +    Trim( qryTabProSpedCodNCM.AsString )                    + '|';                         // Codigo NCM - Código da Nomenclatura Comum do Mercosul {08}
   sAux := sAux + ''      + '|';                                                                            // Código EX, conforme a TIPI
   sAux := sAux + ''      + '|';                                                                            // Código do genero, conforme a Tabela 4.2.1
   sAux := sAux + ''      + '|';                                                                            // Código do Serviço conforme lista do anexo I
   sAux := sAux + ''      + '|';                                                                            // Aliquota do ICMS, operações Internas
 ...
 // joao batista 30-03-2011
 iTRg_C400 := 0;  // EQUIPAMENTO ECF (CÓDIGO 02 e 2D).
 iTRg_C405 := 0;  // REDUÇÃO Z (CÓDIGO 02 e 2D).
 iTRg_C420 := 0;  // REGISTRO DOS TOTALIZADORES PARCIAIS DA REDUÇÃO Z (COD 02 e 2D).
 iTRg_C425 := 0;  // RESUMO DE ITENS DO MOVIMENTO DIÁRIO (CÓDIGO 02 e 2D).
 iTRg_C460 := 0;  // DOCUMENTO FISCAL EMITIDO POR ECF (CÓDIGO 02 e 2D).
 iTRg_C470 := 0;  // ITENS DO DOCUMENTO FISCAL EMITIDO POR ECF (CÓDIGO 02 e 2D).
 iTRg_C490 := 0;  // REGISTRO ANALÍTICO DO MOVIMENTO DIÁRIO (CÓDIGO 02 e 2D).

 // joao batista 23-03-2011
 iTRg_H010 := 0;  // Inventário
 ...
 While Not qryMovNotSped.Eof Do
  Begin
   ...
   sAux := sAux + FormatSpace( qryMovNotSpedTipNot.AsString, 01 )              + '|';                           // Tipo de Operação {0=Entrada ; 1=Saida}
   sAux := sAux + FormatSpace( qryMovNotSpedTipEmt.AsString, 01 )              + '|';                           // Emitente do Doc  {0=Proprio ; 1=Terceiro}

   if qryMovNotSpedSitNot.AsString = '00' Then  // Situação Normal
    sAux := sAux + Trim( qryMovNotSpedCodPrt.AsString )                        + '|'                            // Código do Participante {Existente no Registro 0150} {60}
   Else                                        // Situação Cancelada
    sAux := sAux + ''     + '|';

   sAux := sAux + FormatSpace( qryMovNotSpedModelo.AsString, 02 )              + '|';                           // Codigo do Modelo do Documento Fiscal
   sAux := sAux + FormatSpace( qryMovNotSpedSitNot.AsString, 02 )              + '|';                           // Código da Situação do Documento Fiscal {00=Normal ; 02 = Cancelada}
   sAux := sAux +        Trim( qryMovNotSpedSerie.AsString )                   + '|';                           // Série do Documento {03}
   sAux := sAux +     StrZero( qryMovNotSpedCodNot.AsString, 09 )              + '|';                           // Número do Documento Fiscal

   if qryMovNotSpedSitNot.AsString = '00' Then  // Situação Normal
    Begin
     sAux := sAux + Se( qryMovNotSpedChvNFe.AsString <> '', StrZero( qryMovNotSpedChvNFe.AsString, 44 ), '' )     + '|';  // Chave da Nota Fiscal Eletronica
     sAux := sAux + FormatDateTime( 'ddmmyyyy', qryMovNotSpedDatNot.AsDateTime ) + '|';                         // Inicio das Informações
     sAux := sAux + FormatDateTime( 'ddmmyyyy', qryMovNotSpedDatOpe.AsDateTime ) + '|';                         // Final  das Informações
     ...
     sAux := sAux + ''      + '|';                                                                              // Abatimento não Tributado
     ...
     // campos serão apresentados com valor zerado, pq se não deverá ser informado na totalização
     // da apuração do imcs no bloco E110
   ...
   if( qryMovNotSpedForPag.AsString = '1' ) and ( qryMovNotSpedSitNot.AsString = '00' ) Then  // Situação Normal
    Begin
     ...
     sAux := sAux + FormatSpace( qryMovNotSpedTipEmt.AsString, 01 )        + '|';         // Emitente do Título  {0=Proprio ; 1=Terceiro}
     sAux := sAux + '00'                                                   + '|';         // Tipo de Titulo {00=Duplicata , 01=Cheque, 02=Promissoria, 03=Recibo, 9=Outros}
     sAux := sAux + ''                                                     + '|';         // Descrição Complementar do Titulo de Credito {Tamanho Indefinido}
   ...
   While Not qryIteNotSped.Eof Do
    Begin
     ...
     sAux := sAux +        Trim( qryIteNotSpedCmpPro.AsString )                  + '|';       // 04. Descrição Complemente {Tamanho Indefinido}
     ...
     // campos serão apresentados com valor zerado, pq se não deverá ser informado na totalização
     // da apuração do imcs no bloco E110
     //sAux := sAux +     StrZero( qryIteNotSpedSitTri.AsString, 03 )              + '|';       // 10. Situação Tributária
     //sAux := sAux +     StrZero( qryIteNotSpedCodCfo.AsString, 04 )              + '|';       // 11. CFOP
     if qryIteNotSpedValIcm.AsFloat > 0 then begin
       sAux := sAux +     '040'                                                + '|';       // 02. Situação Tributária
     ...
     sAux := sAux + ''      + '|';                                                        // 12. Natureza de Operação {Existente no Registro 0400}
   ...
   While Not qryResEst.Eof Do
    Begin
     ...
     // campos serão apresentados com valor zerado, pq se não deverá ser informado na totalização
     // da apuração do imcs no bloco E110
     //sAux := sAux +     StrZero( qryResEstSitTri.AsString, 03 )              + '|';       // 02. Situação Tributária
     ...
     sAux := sAux + FormatFloat( '0.00', qryResEstValRed.AsFloat )           + '|';       // 10. Valor não Trib. em função da Redução
     sAux := sAux + '0'     + '|';                                                        // 11. VLR_IPI
     sAux := sAux + ''      + '|';                                                        // 12. Código da Observação do Lançamento fiscal (Campo 02 do Registro 0460) ; By Renildo ; 09.03.2010 ; 11:09
 ...
 While Not qryEcfSped.Eof Do
    Begin
     ...
     sAux := sAux + qryEcfSpedModelo.AsString                         + '|';                 // 02. Código do modelo do documento fiscal, conforme a Tabela 4.1.1
     sAux := sAux + FrTelIni.CompECF.ModeloStr                       + '|';                 // 03. Modelo do equipamento
     sAux := sAux + qryEcfSpedCodECFf.AsString                        + '|';                 // 04. Número de série de fabricação do ECF
     sAux := sAux + qryEcfSpedCodECFp.AsString                        + '|';                 // 05. Número do caixa atribuído ao ECF
     ...
     // joao batista 01-04-2011 - desativado, pq não traz nenhum registro. bug não identificado.
     {
     ...
     qryReducaoZSped.First;
      ...
      While Not qryReducaoZSped.Eof Do
        Begin
         ...
         sAux := sAux + FormatDateTime( 'ddmmyyyy', qryReducaoZSpedDatEmi.AsDateTime )    + '|';  // 02. Data do movimento a que se refere a Redução Z
         sAux := sAux + qryReducaoZSpedGTRnc.AsString                                     + '|';  // 03. Posição do Contador de Reinício de Operação
         sAux := sAux + qryReducaoZSpedReducaoZ.AsString                                  + '|';  // 04. Posição do Contador de Redução Z
         sAux := sAux + qryReducaoZSpedDocIni.AsString                                    + '|';  // 05. Número do Contador de Ordem de Operação do último documento emitido no dia. (Número do COO na Redução Z)
          ...
          While Not qryTotParRedZSped.Eof Do
            Begin
             ...
             if qryTotParRedZSpedGTPrc.AsFloat > 0 then begin
               ...
               if qryTotParRedZSpedNrTot.AsInteger <> 0 then                                 // 02. Código do totalizador, conforme Tabela 4.4.6
               ...
               sAux := sAux + FormatFloat( '0.00', qryTotParRedZSpedGTPrc.AsFloat )  + '|';  // 03. Valor acumulado no totalizador, relativo à respectiva Redução Z.

               if qryTotParRedZSpedNrTot.AsInteger <> 0 then                                 // OC // 04. Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva
                 sAux := sAux + StrZero(qryTotParRedZSpedNrTot.AsString, 2)          + '|'
               else
                 sAux := sAux + ''                                                   + '|';

               sAux := sAux + ''                                                     + '|';  // OC // 05. Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
         ...
         qryIteMovDiaSped.First;
          ...
          While Not qryIteMovDiaSped.Eof Do
            Begin
             ...
             sAux := sAux + Trim( qryIteMovDiaSpedCodPro.AsString )                + '|';  // 02. Código do item (campo 02 do Registro 0200)
         ...
         {
         Regra geral, o perfil “A” determina a apresentação dos registros mais detalhados e o perfil “B”
         trata as informações de forma sintética (totalizações por período:
           ...
           qryDocFisSped.First;
            ...
            While Not qryDocFisSped.Eof Do
              Begin
               ...
               sAux := sAux + qryDocFisSpedCodMod.AsString + '|'; // 02. Código do modelo do documento fiscal, conforme a Tabela 4.1.1
               sAux := sAux + qryDocFisSpedCodSit.AsString + '|'; // 03. Código da situação do documento fiscal, conforme a Tabela 4.1.2
               sAux := sAux + qryDocFisSpedNumDoc.AsString + '|'; // 04. Número do documento fiscal (COO)

               if (qryDocFisSpedCodSit.AsString = '00') and (qryDocFisSpedVlrDoc.AsFloat > 0) then begin  // SitDoc = Regular
                 sAux := sAux + FormatDateTime( 'ddmmyyyy', qryDocFisSpedDatDoc.AsDateTime) + '|'; // 05. Data da emissão do documento fiscal
                 ...
                 sAux := sAux + ''                                                          + '|'; // 05. Data da emissão do documento fiscal
               ...
               qryIteDocFisSped.First;
                ...
                While Not qryIteDocFisSped.Eof Do
                  Begin
                   ...
                   sAux := sAux + qryIteDocFisSpedCodItem.AsString                            + '|'; // 02. Código do item (campo 02 do Registro 0200)
                   ...
                   sAux := sAux + qryIteDocFisSpedCodStb.AsString                             + '|'; // 07. Código da Situação Tributária, conforme a Tabela indicada no item 4.3.1.
                   sAux := sAux + qryIteDocFisSpedCodCfo.AsString                             + '|'; // 08. Código Fiscal de Operação e Prestação
                   sAux := sAux + iif( qryIteDocFisSpedAliqIcms.AsFloat > 0, FormatFloat( '0.00', qryIteDocFisSpedAliqIcms.AsFloat), '')      + '|'; // OC // 09. Alíquota do ICMS – Carga tributária efetiva em percentual
         ...
         qryResDocFisSped.First;
          ...
          While Not qryResDocFisSped.Eof Do
            Begin
             ...
             sAux := sAux + qryResDocFisSpedSitTri.AsString                             + '|'; // 02. Código da Situação Tributária, conforme a Tabela indicada no item 4.3.1
             sAux := sAux + qryResDocFisSpedCodCfo.AsString                             + '|'; // 03. Código Fiscal de Operação e Prestação
             sAux := sAux + iif( qryResDocFisSpedAlqIcm.AsFloat > 0, FormatFloat( '0.00', qryResDocFisSpedAlqIcm.AsFloat), '')   + '|'; // OC // 04. Alíquota do ICMS
             sAux := sAux + FormatFloat( '0.00', qryResDocFisSpedValPro.AsFloat)        + '|'; // 05. Valor da operação correspondente à combinação de CST_ICMS, CFOP, e alíquota do ICMS, incluídas as despesas acessórias e acréscimos
             sAux := sAux + FormatFloat( '0.00', qryResDocFisSpedBasIcm.AsFloat)        + '|'; // 06. Valor acumulado da base de cálculo do ICMS, referente à combinação de CST_ICMS, CFOP, e alíquota do ICMS.
             sAux := sAux + FormatFloat( '0.00', qryResDocFisSpedValIcm.AsFloat)        + '|'; // 07. Valor acumulado do ICMS, referente à combinação de CST_ICMS, CFOP e alíquota do ICMS.
             sAux := sAux + ''                                                          + '|'; // OC // 08. Código da observação do lançamento fiscal (campo 02 do Registro 0460)
 ...
 // 3. BLOCO D - Documentos Fiscais II - Serviços (ICMS) {NF_SRV_TRANSP_07 ; CNH_FRETE_ROD_08 ; AVULSO_8B ; AQUAVIARIO_09 ; AEREO_10 ; FERROV_11 ; _26 ; _27 ; CT-e_57}
 ...
 // 4. BLOCO E - Apuração do ICMS e do IPI
 ...
 // 4.2. Registro E100 - Periodo da apuração do ICMS

 iLin := iLin + 1;
 iLnG := iLnG + 1;

 sAux := '|'  + 'E100'  + '|';                                                            // Tipo de Registro
 sAux := sAux + FormatDateTime( 'ddmmyyyy', dlDatIni ) + '|';                             // Inicio da Apuração
 sAux := sAux + FormatDateTime( 'ddmmyyyy', dlDatFin ) + '|';                             // Final  da Apuração

 WriteLn( F, sAux );

 // 4.3. Registro E110 - Apuração do ICMS - Operações Próprias
 ...
 // 5.2. Registro H005 - Totais do Inventário ; By Renildo ; 09.03.2010 ; 15:36
 ...
 sAux := sAux + FormatDateTime( 'ddmmyyyy', dlDatFin )             + '|';                 // Data do Inventário
 sAux := sAux + FormatFloat( '0.00', qryInvTotVlrTotIvt.AsFloat )  + '|';                 // Valor Total do Estoque

 WriteLn( F, sAux );

 // 5.3 Registro H010 - Inventário {Por Produto} ; By Renildo ; 09.03.2010 ; 15:42
 // joao batista 23-03-2011
 ...
 While Not qryInventario.Eof Do
  Begin
   ...
   sAux := sAux + Trim( qryInventarioCodPro.AsString )                     + '|';       // 02. Código Item
   ...
   sAux := sAux + FormatFloat( '0.000000', qryInventarioVlrUnit.AsFloat )  + '|';       // 05. Vlr Unitário Item
   ...
   sAux := sAux + ''                                                       + '|';       // 08. Código do participante
   sAux := sAux + ''                                                       + '|';       // 09. Descrição complementar
   sAux := sAux + Trim( qryTabEmpCodCtaInvEFD.AsString )                   + '|';       // 10. Código da conta analítica contábil debitada/creditada
 ...
 sAux := sAux + '0000'  + '|';                                                            // Registro 0000 - Abertura do Arquivo e Identificação da Entidade
 ...
 sAux := '|'  + '9900'                + '|';                                              // Tipo de Registro
 sAux := sAux + '0200'                + '|';                                              // Registro 0200 - Tabela de Identificação do Item
 ...
 // joao batista 30-03-2011
 // inclusão dos blocos C400 a C490
 iLin := iLin + 1;
 iLnG := iLnG + 1;

 sAux := '|'  + '9900'                + '|';                                              // Tipo de Registro
 sAux := sAux + 'C400'                + '|';                                              // Registro C400 - Resumo Analitico EQUIPAMENTO ECF (CÓDIGO 02 e 2D).
 ...
 sAux := '|'  + '9900'  + '|';                                                            // Tipo de Registro
 sAux := sAux + 'E100'  + '|';                                                            // Registro E100 - Periodo de Apuração do ICMS
 sAux := sAux + '1'     + '|';                                                            // Total de Registros do Tipo
 ...
 sAux := '|'  + '9900'  + '|';                                                            // Tipo de Registro
 sAux := sAux + 'E110'  + '|';                                                            // Registro E110 - Apuração do ICMS - Operações Próprias
 sAux := sAux + '1'     + '|';                                                            // Total de Registros do Tipo
 ...
 sAux := '|'  + '9900'  + '|';                                                            // Tipo de Registro
 sAux := sAux + 'H005'  + '|';                                                            // Registro H005 - Totais do Inventário
 sAux := sAux + '1'     + '|';                                                            // Total de Registros do Tipo
 ...
 sAux := '|'  + '9900'  + '|';                                                            // Tipo de Registro
 sAux := sAux + 'H010'  + '|';                                                            // Registro H010 - Totais do Inventário
...
begin
 ...
 while not qryTabpro.Eof do begin
   ...
   // joao batista 18-07-2011
   // forma de identificação do produto que teve a qtde e a data de estoque alterada
   ...
   E2.QTDE_EST := qryTabPro.FieldByName('Qtde_Est').AsFloat;                // Quantidade em estoque referente a sua última atualização.
