...
begin
 ...
 //-----------------------------------------------------------------------------
 // Inicializa o status
 FrGBNFe.fMudaVersao( FrPar.cbb2.ItemIndex, gModelo );                                  // Mudança de versão
 ...
 // Define os parametros de relatórios
 FrGBNFe.pDefineRel();
...
begin
 ...
 if not DMFD.FDQuery5.IsEmpty then
  begin
   ...
   if Application.MessageBox( PWideChar('Confirme a Exclusão desta CCe?'), 'GBNFe - Exclusão de CC-e', mb_IconInformation + mb_YesNo ) = IdNo then
    exit;
...
begin
 ...
 if Application.MessageBox( PWideChar('Confirme a impressão da CCe ?'), 'GBNFe - Impressão CC-e', mb_IconInformation + mb_YesNo ) = IdYes then
  begin
   p_ImprimeCCe();
  end;
...
begin
 ...
 xAux_XML := trim(gCamXml) +                                                    // Caminho do arquivo log (gCamXml contém o caminho padrão) ex: c:\Sistemas\GBNFe\Arq\Emp001\Xml\
             FormatDateTime('yyyymm', vdhRegEvento) +                           // Ano, mês
