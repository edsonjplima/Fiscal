...
begin
 ...
 if ( (Trim(edt_Database_NFe.Text) = '') or
  ...
  begin
   ...
   Application.MessageBox(PWideChar('Atenção, um ou mais item obrigatório está vazio!' + Char(13) +
                                    'O sistema será fechado!'), 'FireDAC', MB_ICONEXCLAMATION+mb_ok );
   Halt;
  ...
  begin
   ...
   if not ( FileExists(gCamExe + 'GBNFe.ini') ) then
    begin
     ...
     Application.MessageBox(PWideChar('Atenção, arquivo ini não foi encontrado!' + Char(13) +
                                      'O sistema será fechado!'), 'FireDAC', MB_ICONEXCLAMATION+mb_ok );
     Halt;
