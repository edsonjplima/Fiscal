...
begin
 ...
 if ( dtp1.Date > Now ) then
  begin
   Application.MessageBox(PWideChar('Data inicial não pode ser maior que a data atual!'), 'Atenção', MB_ICONINFORMATION );
   dtp1.SetFocus;
  end
 else if ( dtp1.Date > dtp2.Date ) then
  begin
   Application.MessageBox(PWideChar('Data inicial não pode ser maior que a data final!'), 'Atenção', MB_ICONINFORMATION );
   dtp1.SetFocus;
  end;
...
begin
 ...
 if ( dtp2.Date > Now ) then
  begin
   Application.MessageBox(PWideChar('Data final não pode ser maior que a data de hoje!'), 'Atenção', MB_ICONINFORMATION );
   dtp2.SetFocus;
  end
 else if ( dtp2.Date < dtp1.Date ) then
  begin
   Application.MessageBox(PWideChar('Data final não pode ser menor que a data inicial!'), 'Atenção', MB_ICONINFORMATION );
   dtp2.SetFocus;
  end;
...
begin
 ...
 // Pega o mês, ano e data inicial e final do período para analise de pastas
 ...
 // Se a pasta do mês da data final não existir o sistema gbnfe cria
 if not DirectoryExists(vArq1) then
  begin
   CreateDir(vArq1);
  end;

 // Se a pasta contab do período não existir o sistema gbnfe cria
