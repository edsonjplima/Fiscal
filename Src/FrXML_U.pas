...
type
  ...
  public
    { Public declarations }
    procedure MarcaBloco( cxTLX : TcxTreeList; blMarca : Boolean; blTodos : Boolean = False ); // Marca bloco de seleção TreeList
...
begin
 ...
 if not( FrGBNFe.fTemSel( FrGBNFe.cxTL, FrGBNFe.cxTLbSel ) ) then exit;         // verifica se tem item selecionado na treelist lá das notas

 for X := 0 to FrGBNFe.cxTL.Count -1 do                                         // Percorre a treelist

  begin

   if ( FrGBNFe.cxTL.Items[X].Texts[FrGBNFe.cxTLbSel.ItemIndex] = 'True' ) then // Verifica se o item está selecionado
...
begin
 ...
 case PageControl1.TabIndex of
            ...
            if (vGNoDhE and vGBoDhE) then
             begin
              if ( (vNomXML[i] = 'T') or (vNomXML[i] = '<') )  then
               begin
                gAnoMes := Copy(vDataEmi, 1, 4) + Copy(vDataEmi, 6, 2);         // copia o ano e o mes para variável global.
                vDataEmi := '';
                FrGBNFe.MemoResp.Lines.Text := UTF8Encode(DMFD.FDQuery13['xml_nota']    +// by Edson ; 2013-12-13T1555 ; acrescido as outras notas
                                                          DMFD.FDQuery13['xml_nota1']   +
                                                          DMFD.FDQuery13['xml_nota2']   +
  ...
  1 :                                                                           // Salva um período confome data ini e data fim
...
begin
 ...
 if ( cxdtp1.Date > cxdtp2.Date ) then
  begin
   ...
   Application.Messagebox('A data inicial está maior que a data final!','Atenção!',mb_iconstop+mb_ok);
   cxdtp1.SetFocus;
...
begin
 ...
 if ( cxdtp2.Date < cxdtp1.Date ) then
  begin
   ...
   Application.Messagebox('A data final está menor que a data inicial!','Atenção!',mb_iconstop+mb_ok);
   cxdtp2.SetFocus;
...
Var
 I : Integer;
begin
 // by Edson ; 2017-7-7T1153 ; Atribui seleção da TreeList e Monta a query das pendentes
