...
var
 ...
 gSoUmaConsulta                                                           : Boolean = False; // Excluir durante consulta
    ...
    begin
     ...
     if xAux <> '' then
      begin
       ...
       xAux := trim(gCamXml) + gAnoMesGer + gTN + trim(xAux) + '-nfe.xml';      // By-EL 10-06-2021 - Antes ==> xAux := trim(gCamLog) + trim(xAux) + '-nfe.xml';
...
begin
 ...
 DMFD.FDQryGer.SQL.Add( 'SET DATEFORMAT dmy                             ' );
...
//------------------------------------------------------------------------------
function TFrGBNFe.fDelFileCnt(Caminho, Mascara: string): boolean;
var
 SearchRec : TSearchRec;
 Busca     : Integer;
...
begin
 ...
 Busca  := FindFirst(Caminho + Mascara, faAnyFile, SearchRec);
