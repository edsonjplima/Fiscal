...
begin
 ...
 while true do
  begin
   if not FrBuscaChave.Atualiza(vRep) then    // Se atualização não der problemas....
    begin
    ...
    begin
     vRep := True;
     if Application.Messagebox('ATT: Erro de lentidão na sefaz!' + Chr(13) +
                               'deseja continuar mesmo assim?','Atenção!', mb_YesNo+mb_ICONQUESTION+mb_DefButton2) = IdNo then
      begin
       Edit_Busca_Chave.SetFocus;
       exit;
      end;
