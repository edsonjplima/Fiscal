...
-- Deve ser utilizada apartir da vers„o 4.14.12.1 do GBNFe
-- GBNFe_V_(Z) 4.14.12.1  - 05. Atualiz. All Proc. 
--             Implica sp_importar - xPed e nItemPed
--*********************************************************
...
CREATE    proc [dbo].[sp_calcula_digito_chave] @codigo_loja int, @nota int, @tpemissao char(1),@demissao datetime, @modelo char(2)
as
...
if not exists (select * from nfe where codigo_loja = @codigo_loja and nnf = @nota and dEmi = @demissao and modelo = @modelo) begin
...
 serie - SÇrie do Documento Fiscal
...
set @chave = @chave + (select (substring(cast(year(demi)as varchar),3,2)) from nfe where codigo_loja = @codigo_loja and nnf = @nota and dEmi = @demissao and modelo = @modelo) 

set @chave = @chave + (select (replicate('0', 2 - len(cast(month(demi) as varchar))) + cast(month(demi) as varchar)) from nfe where codigo_loja = @codigo_loja and nnf = @nota and dEmi = @demissao and modelo = @modelo)

set @chave = @chave + rtrim((select cnpj from emitente where codigo_loja = @codigo_loja)) + @modelo + rtrim(ltrim((select (replicate('0', 3 - len(serie)) + serie ) from nfe where codigo_loja = @codigo_loja and nnf = @nota and dEmi = @demissao and modelo = @modelo)))
...
CREATE   proc [dbo].[sp_exclui_nfe] @codigo_loja int,@nnf varchar(10), @demissao dateTIME, @Modelo char(2)
as

set nocount on

delete from nfe_itens 
where codigo_loja = @codigo_loja and nNF = @nnf and dEmi = @demissao and modelo = @Modelo
...
update t1 set 
        ...
        ,UFSbt                   = t2.UFSbt
        -- by Edson Lima ; 2015/12/17 ; 11:34
        ,Certificado_NumSerie    = t2.Certificado_NumSerie
        ,DANFE_QtdCopias         = t2.DANFE_QtdCopias
        ,DANFE_Visualiza         = t2.DANFE_Visualiza
        ,DANFE_LogoMarca         = t2.DANFE_LogoMarca
        ,DANFE_LocSRV            = t2.DANFE_LocSRV
        ,DANFE_CAMSRV            = t2.DANFE_CAMSRV
        ,Web_Ambiente            = t2.Web_Ambiente
        ,Web_Visualizar          = t2.Web_Visualizar
        ,Proxy_Host              = t2.Proxy_Host
        ,Proxy_Porta             = t2.Proxy_Porta
        ,Proxy_User              = t2.Proxy_User
        ,Proxy_Pass              = t2.Proxy_Pass
        ,Email_NEeMAIL           = t2.Email_NEeMAIL
        ,Email_Host              = t2.Email_Host
        ,Email_Port              = t2.Email_Port
        ,Email_User              = t2.Email_User
        ,Email_Pass              = t2.Email_Pass
        ,Email_SSL               = t2.Email_SSL
        ,Email_Assunto           = t2.Email_Assunto
        ,Email_Assunto_Canc      = t2.Email_Assunto_Canc
        ,Email_CC                = t2.Email_CC 
        ,Email_Mensagem          = t2.Email_Mensagem
        ,Email_Assunto_CCe       = t2.Email_Assunto_CCe
        ,DANFE_HorariodeVerao    = t2.DANFE_HorariodeVerao
        ,DANFE_FusoHorario       = t2.DANFE_FusoHorario
        ,DANFE_UsaHorarioDF      = t2.DANFE_UsaHorarioDF
        ,DANFE_ImportaTxt        = t2.DANFE_ImportaTxt
        ,Certificado_NumSerie2   = t2.Certificado_NumSerie2
        ,CSC                     = t2.CSC
        ,IdCSC                   = t2.IdCSC
...
(        
	...
	,Email_Mensagem
	,Email_Assunto_CCe
	,DANFE_HorariodeVerao
	,DANFE_FusoHorario
	,DANFE_UsaHorarioDF
	,DANFE_ImportaTxt
	,Certificado_NumSerie2
	,CSC
	,IdCSC
         )
select	
        ...
        ,t1.Email_SSL
        ,t1.Email_Assunto
        ,t1.Email_Assunto_Canc
        ,t1.Email_CC
        ,t1.Email_Mensagem
        ,t1.Email_Assunto_CCe
        ,t1.DANFE_HorariodeVerao
        ,t1.DANFE_FusoHorario
        ,t1.DANFE_UsaHorarioDF
        ,t1.DANFE_ImportaTxt
        ,t1.Certificado_NumSerie2
        ,t1.CSC
        ,t1.IdCSC
...
CREATE  proc [dbo].[sp_insere_notas_inutilizadas] @codigo_loja int,@nota int,@ano int,@modelo int,@serie int,@justificativa varchar(255)
as
...
CREATE    proc [dbo].[sp_le_nfe] @codigo_loja int, @nota varchar(06),@demi as datetime, @Modelo char(2)
as
...
if @Modelo = '65'
 ...
 where codigo_loja    = @codigo_loja and
               nnf    = @nota        and
               dEmi   = @demi        and
               modelo = @Modelo      and  DatNFCe is null
...
left  join nfe_volume     t4 on t1.codigo_loja = t4.codigo_loja and t1.demi = t4.demi and t1.nnf = t4.nnf and t1.modelo = t4.modelo
left  join nfe_veiculo    t5 on t1.codigo_loja = t5.codigo_loja and t1.demi = t5.demi and t1.nnf = t5.nnf and t1.modelo = t5.modelo
...
CREATE   proc [dbo].[sp_ler_nfe_textos]  @gCamTxt varchar(400)  -- by Edson Lima ; 4.4.2012 ; 11:22 ; Alterado antes mais confirmado nesta data
...
CREATE                    proc [dbo].[sp_nfe_a_serem_geradas] @codigo_loja int, @Modelo char(2)
as
...
where (t1.codigo_loja          = @codigo_loja)
and   (t1.Modelo               = @Modelo or @Modelo = '')
and
    ( ( (t1.situacao <> '100') and (t1.situacao <> '150')  and (t1.situacao <> '101') and (t1.situacao <> '135')  and
        ...
        (t1.situacao <> '110') and (t1.situacao <> 'EPEC'))
or
        (t1.situacao is null) )

        -- ( IsNull( t1.chave_nfe, '' )  = '' or (t1.chave_nfe = '0') or (t1.situacao = 'ENVI'))
        -- By Edson Lima 18.4.2012 --> Antes - and (t1.chave_nfe is null)
        -- By Edson Lima 8.11.2012 --> Acrescido de - or (t1.situacao = 'ENVI')
...
CREATE        proc [dbo].[sp_nfe_canceladas] @codigo_loja int, @data_inicio datetime, @data_final datetime, @nota bigint, @destinatario int, @Modelo char(2)   -- By Edson Lima 3.2.2012 ---> linha inserida - 2012-3-6 --> alterada
as
...
and   ( (IsNull(t1.situacao, '') = '101') or (IsNull(t1.situacao, '') = '135') or (IsNull(t1.situacao, '') = '136') or
        (IsNull(t1.situacao, '') = '151') or (IsNull(t1.situacao, '') = '155') )  -- by Edson ; 2013/03/19 ; 14:49 ; o Status 101 continua somente para efeito de compatibilidade 
...
CREATE        proc [dbo].[sp_nfe_denegadas] @codigo_loja int, @data_inicio datetime, @data_final datetime, @nota bigint, @destinatario int, @Modelo char(2)   -- By Edson Lima 3.2.2012 ---> linha inserida - 2013-3-6 --> alterada
as
...
order by t1.serie, t1.dEmi, t1.nNF
...
CREATE       proc [dbo].[sp_nfe_em_contingencia] @codigo_loja int, @Modelo char(2)
as
...
CREATE    proc [dbo].[sp_nfe_inutilizadas] @codigo_loja int, @Modelo int
as
...
order by t3.data_inclusao, t3.modelo
...
CREATE          proc [dbo].[sp_nfe_transmitidas] @codigo_loja int, @data_inicio datetime, @data_final datetime, @nota bigint, @destinatario int, @Modelo char(2)
as
