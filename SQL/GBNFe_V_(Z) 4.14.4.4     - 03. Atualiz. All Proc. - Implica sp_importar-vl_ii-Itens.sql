...
CREATE proc [dbo].[sp_calcula_digito_chave] @codigo_loja int, @nota int, @tpemissao char(1),@demissao datetime
as
...
CREATE proc [dbo].[sp_exclui_nfe] @codigo_loja int,@nnf varchar(10), @demissao dateTIME
as
...
update t1 set 
	...
	,uf = t2.uf
	,ufSbt = t2.ufSbt                                  -- by Edson Lima ; 2013/02/25 ; 09:06
...
(
	...
	codigo_uf,
	uf,
	ufSbt,                                             -- by Edson Lima ; 2013/02/25 ; 09:07
	insc_estadual,
	insc_estadual_subs,
	suframa,
	codigo_pais,
...
select	
	...
	t1.codigo_uf,
	t1.uf,
	t1.ufSbt,                                           -- by Edson Lima ; 2013/02/25 ; 09:07
	t1.insc_estadual,
...
(
	...
	cst,
	reducao_icms,
	pis_cst,						-- by Edson Lima ; 02.01.2013 ; 10:45 - linha incluída
	pis_base_calculo,
	pis_percentual,
	pis_valor,
	cofins_cst,
...
select
	...
	t1.cst,
	t1.reducao_icms,
	t1.pis_cst,					-- by Edson Lima ; 02.01.2013 ; 10:45 - linha incluída
...
CREATE  proc [dbo].[sp_le_nfe] @codigo_loja int, @nota varchar(06),@demi as datetime
as
...
------RECONFIGURE
------EXEC sp_configure æxp_cmdshellÆ, 1
------RECONFIGURE;
------GO
...
set @dir = rtrim(@gCamTxt)   -- -- by Edson Lima ; 4.4.2012 ; 11:22 ; Alterado - Antes tinha um texto com o caminho do diret¾rio onde deveria achar os arquivos
...
CREATE            proc [dbo].[sp_nfe_a_serem_geradas] @codigo_loja int
as
...
CREATE   proc [dbo].[sp_nfe_canceladas] @codigo_loja int, @data_inicio datetime, @data_final datetime, @nota bigint, @destinatario int   -- By Edson Lima 3.2.2012 ---> linha inserida - 2012-3-6 --> alterada
as
...
,t1.chave_nfe						as nfe_chave_nfe
,t1.codigo_destinatario					as nfe_codigo_destinatario
,(t1.situacao + '-' + isnull(t1.motivo,''))		as nfe_situacao
,t1.data_hora_recebimento						as nfe_data_hora_recebimento
,t1.nr_dpec										as nfe_nr_dpec
...
and (t1.codigo_destinatario = @destinatario or @destinatario = 0) -- By Edson Lima ; 2012/03/06 ---> filtra por destinatário
...
CREATE    proc [dbo].[sp_nfe_denegadas] @codigo_loja int, @data_inicio datetime, @data_final datetime, @nota bigint, @destinatario int   -- By Edson Lima 3.2.2012 ---> linha inserida - 2013-3-6 --> alterada
as
...
CREATE  proc [dbo].[sp_nfe_em_contingencia] @codigo_loja int
as
...
CREATE proc [dbo].[sp_nfe_inutilizadas] @codigo_loja int
as
...
CREATE     proc [dbo].[sp_nfe_transmitidas] @codigo_loja int, @data_inicio datetime, @data_final datetime, @nota bigint, @destinatario int
as
